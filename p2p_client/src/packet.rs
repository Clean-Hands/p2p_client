...
pub fn decode_packet(packet_bytes: [u8; 512]) -> Packet {
    ...
    // TODO:
    // assert_eq! is not the desired behavior of this function (want to verify packet so that we 
    // can request the packet again if needed). assert_eq! will end the running of the program instead
    assert_eq!(chunk_hash, packet.chunk_hash, "Computed chunk hash does not match chunk hash within packet.");
...
mod tests {
    ...
    fn test_neq_packet() {
        let expected = Packet {
            data_length: 87, 
            filename_len: 8, 
            filename: String::from("different packet.txt"),
            data: vec![1, 2, 3, 4, 5, 6, 5, 4, 3, 2, 1],
            chunk_hash: [225, 12, 171, 217, 101, 208, 53, 140, 202, 193, 162, 185, 202, 9, 198, 105, 184, 61, 132, 233, 44, 148, 213, 111, 38, 87, 245, 175, 76, 14, 186, 117],
            file_hash: [7; 32],
        };
